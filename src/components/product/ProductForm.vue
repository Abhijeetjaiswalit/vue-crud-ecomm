    ...
    <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12">
      ...
      <div class="form-group">
        ...
        <select
          ...
          name="manufacturer"
          :class="{'form-control': true, 'error': errors.has('manufacturer') }">
            <option v-for="manufacturer in manufacturers" :value="manufacturer" :key="manufacturer" >{{manufacturer.name}}</option>
        </select>
...
export default {
  ...
  created() {
    console.log("this.model.edit", this.model);
  },
  methods: {
    saveProduct() {
        ...
        .then(result => {
          if (!result) {
            this.$store.commit(ERROR_MSG, {
              type: "error",
              title: "Validation!",
              content: "Please ensure that the form is valid."
            });
            return;
          }
